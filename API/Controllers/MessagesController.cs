...
public class MessagesController(IMessageRepository messageRepository, IUserRepository userRepository, IMapper mapper) : BaseApiController
{
    ...
    [HttpGet]
    public async Task<ActionResult<IEnumerable<MessageDto>>> GetMessagesForUsers(
    ...
    public async Task<ActionResult> DeleteMessage(int id)
    {
        ...
        if (message.RecipientUsername == username) message.RecipientDetleted = true;

        if (message is {SenderDeleted: true, RecipientDetleted: true})
        {
